	...
	grunt.initConfig({
		...
		concat: {
			...
			head: {
				src: [
					...
					//'js/libs/angular/1.2.14/angular-sanitize.min.js',
		...
		uglify: {
			...
			tilbudswizard: {
				...
				dest: '../scripts/tilbudswizard.min.js'
			}
		},
		compass: {
			dist: {
		...
		watch: {
			scripts: {
				files: ['<%= concat.tilbudswizard.src %>',
					'<%= concat.head.src %>'],
				tasks: ['concat']
			},
			images: {
